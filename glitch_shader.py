...
def main():
    ...
    # pr.ffi.NULL for the vertex shader tells raylib to use its default one
    shader = pr.load_shader_from_memory(pr.ffi.NULL, FRAGMENT_SHADER) # type: ignore
    ...
    # Uniform values must be passed as cffi pointers, not plain Python
    # numbers/lists. Reuse the same buffers each frame instead of
    # reallocating them every call.
    time_buf = pr.ffi.new("float *", 0.0)
    intensity_buf = pr.ffi.new("float *", 0.0)
    resolution_buf = pr.ffi.new("float[2]", (float(WIDTH), float(HEIGHT)))
    pr.set_shader_value(shader, loc_resolution, resolution_buf, pr.ShaderUniformDataType.SHADER_UNIFORM_VEC2)
    ...
    while not pr.window_should_close():
        ...
        time_buf[0] = elapsed
        intensity_buf[0] = intensity
        pr.set_shader_value(shader, loc_time, time_buf, pr.ShaderUniformDataType.SHADER_UNIFORM_FLOAT)
        pr.set_shader_value(shader, loc_intensity, intensity_buf, pr.ShaderUniformDataType.SHADER_UNIFORM_FLOAT)
